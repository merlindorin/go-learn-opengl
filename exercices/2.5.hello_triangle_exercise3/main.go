...
const (
	width  = 800
	height = 600
	title  = "LearnOpenGL: hello_triangle_exercise3"
)
...
func main() {
	...
	fragmentShaderOrange, err := utils.NewShaderFromFile("fragment_shader_orange.glsl", utils.FRAGMENT_SHADER)
	if err != nil {
		logger.Fatal("failed to create shader", zap.Error(err))
	}

	fragmentShaderYellow, err := utils.NewShaderFromFile("fragment_shader_yellow.glsl", utils.FRAGMENT_SHADER)
	if err != nil {
		logger.Fatal("failed to create shader", zap.Error(err))
	}

	shaderProgramOrange, err := utils.NewProgram(vertexShader, fragmentShaderOrange)
	if err != nil {
		logger.Fatal("failed to create program", zap.Error(err))
	}
	defer shaderProgramOrange.Delete()

	shaderProgramYellow, err := utils.NewProgram(vertexShader, fragmentShaderYellow)
	if err != nil {
		logger.Fatal("failed to create program", zap.Error(err))
	}
	defer shaderProgramYellow.Delete()
	...
	for !window.ShouldClose() {
		...
		// Draw the first triangle using the vertex and orange fragment shader
		shaderProgramOrange.Use()
		gl.BindVertexArray(VAOs[0])
		gl.DrawArrays(gl.TRIANGLES, 0, 3)

		// Draw the second triangle using the vertex and yellow fragment shader
